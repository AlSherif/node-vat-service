...
export const createVatValidationController = (configuration: Configuration) => {
  const externalVatServices: Array<ExternalVatValidationService> = [];
  externalVatServices.push(
    new EUVatValidationService(configuration.apiUrl['EUVatValidationService']),
  );
  externalVatServices.push(
    new CHVatValidationService(configuration.apiUrl['CHVatValidationService']),
  );
  ...
  return async (req: Request, res: Response) => {
    ...
    try {
      simpleRequestValidationSchema.parse(vatRequest);
    } catch (error) {
    ...
    for (const service of externalVatServices) {
      const temp = service.getSupportedCountry(countryCode);
      if (temp) {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        details: isValid
          ...
          : 'VAT number marked as invalid by the external service.',
      });
    } catch (error) {
